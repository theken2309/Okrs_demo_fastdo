class Mention {
  ...
  * @param {Function} [settings.template] - The template function outputs the innerHTML of the 
  */
  constructor(settings) {
    this.options = settings.options || []
    this.input = settings.input
    this.reverse = settings.reverse
    this.returnHtml = settings.returnHtml
    ...
    this.upDownStay = 0
    this.wordAtCursor = {}
    this.send = settings.send || function (empty) { }
    this.update = settings.update || function (option, content) { }
  ...
  */
  match(word, option) {
    ...
    return optionText.toLowerCase().includes(word.toLowerCase());
  }
  ...
  */
  listen() {
    ...
    this.html.options.forEach(o => {
      o.addEventListener('click', e => this.onEventOptionClick(e.currentTarget));
    })
    this.html.button.addEventListener('click', () => this.OnSend())
  }
  ...
  */
  onEventInput() {
    this.update(null, this.html.input.value);
  }
 ...
 */
  onEventOptionClick(optionEle) {
    var option = JSON.parse(optionEle.getAttribute('mentiondata'))
    var name = option.name
    var splitInputValue = this.html.input.value.split('')
    splitInputValue.splice(this.wordAtCursor.index, this.wordAtCursor.word.length, "@" + name + " ")
    this.html.input.value = splitInputValue.join('')
    this.html.input.focus()
    this.setCursorPosition(this.wordAtCursor.index + name.length + 1)
    this.toggleOptionList(false)
    this.update(option, this.html.input.value)
  }
  ...
  */
  showHideOptions() {
    for (var option in this.options) {
      ...
      if (this.options[option].name.toLowerCase().includes(word))
        this.html.options[option].classList.add('show')
    }
  }
